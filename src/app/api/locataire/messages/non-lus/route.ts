...
/**
 * @swagger
 * /api/locataire/messages/non-lus:
 *   get:
...
export async function GET(request: NextRequest) {
  return withAuth(async (req: AuthenticatedRequest) => {
    try {
      ...
      const count = await messageRepository.countUnreadByUser(req.user.id);

      return NextResponse.json(
        ApiResponse.success({ nombreMessagesNonLus: count }, 'Nombre de messages non lus'),
        { status: 200 }
      );
    } catch (error) {
