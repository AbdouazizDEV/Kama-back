...
test_endpoint() {
  ...
  if [ "$HTTP_CODE" -ge 200 ] && [ "$HTTP_CODE" -lt 300 ]; then
    ...
    echo "$BODY" | python3 -m json.tool 2>/dev/null | head -15 || echo "$BODY" | head -3
  else
...
# Récupérer les IDs
RESERVATIONS=$(curl -s -X GET "$BASE_URL/api/proprietaire/reservations" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "import sys, json; data = json.load(sys.stdin); print(data.get('data', []))" 2>/dev/null)

# Trouver une réservation EN_ATTENTE
RESERVATION_ID=$(echo "$RESERVATIONS" | python3 -c "import sys, json; reservations = json.load(sys.stdin); en_attente = [r for r in reservations if r.get('statut') == 'EN_ATTENTE']; print(en_attente[0]['id'] if en_attente else '')" 2>/dev/null)

# Si pas de réservation en attente, utiliser la première
if [ -z "$RESERVATION_ID" ]; then
  RESERVATION_ID=$(echo "$RESERVATIONS" | python3 -c "import sys, json; reservations = json.load(sys.stdin); print(reservations[0]['id'] if reservations else '')" 2>/dev/null)
fi
...
if [ -n "$RESERVATION_ID" ]; then
  ...
  RESERVATION_STATUT=$(echo "$RESERVATIONS" | python3 -c "import sys, json; reservations = json.load(sys.stdin); r = next((r for r in reservations if r.get('id') == sys.argv[1]), None); print(r.get('statut', '') if r else '')" "$RESERVATION_ID" 2>/dev/null)
  
  if [ "$RESERVATION_STATUT" = "EN_ATTENTE" ]; then
    ...
    test_endpoint "POST" "/api/proprietaire/reservations/$RESERVATION_ID/signer" "" "POST /proprietaire/reservations/{id}/signer - Signer le contrat"
    
    # Créer une nouvelle réservation pour tester refuser
    echo "📅 Création d'une nouvelle réservation pour tester le refus..."
    npx tsx scripts/create-test-data-proprietaire.ts > /dev/null 2>&1
    sleep 2
    
    # Récupérer la nouvelle réservation
    ...
    echo "   ⚠️  Réservation déjà acceptée"
    if [ "$RESERVATION_STATUT" = "ACCEPTEE" ]; then
      test_endpoint "POST" "/api/proprietaire/reservations/$RESERVATION_ID/signer" "" "POST /proprietaire/reservations/{id}/signer - Signer le contrat"
    fi
  fi
else
...
if [ -n "$PAIEMENT_ID" ]; then
  ...
  echo "⚠️  Aucun paiement trouvé"
fi

# Pour restituer caution, il faut une réservation terminée
echo "   ⚠️  POST /proprietaire/paiements/caution/restituer nécessite une réservation TERMINEE"
...
if [ -n "$CONVERSATION_ID" ]; then
  ...
  # Récupérer le locataire ID pour envoyer un message
  LOCATAIRE_ID=$(curl -s -X GET "$BASE_URL/api/proprietaire/reservations/$CONVERSATION_ID/locataire" \
    -H "Authorization: Bearer $TOKEN" | python3 -c "import sys, json; data = json.load(sys.stdin); print(data.get('data', {}).get('id', ''))" 2>/dev/null)
  
  if [ -n "$LOCATAIRE_ID" ]; then
    test_endpoint "POST" "/api/proprietaire/messages" "{\"reservationId\":\"$CONVERSATION_ID\",\"destinataireId\":\"$LOCATAIRE_ID\",\"contenu\":\"Message de test du propriétaire\"}" "POST /proprietaire/messages - Envoyer un message"
    
    # Récupérer le message ID pour marquer comme lu
    MESSAGE_ID=$(curl -s -X GET "$BASE_URL/api/proprietaire/messages/$CONVERSATION_ID" \
      -H "Authorization: Bearer $TOKEN" | python3 -c "import sys, json; data = json.load(sys.stdin); messages = data.get('data', {}).get('messages', []); print(messages[-1]['id'] if messages else '')" 2>/dev/null)
    
    # Récupérer un message envoyé par le locataire (non lu)
    MESSAGE_ID=$(curl -s -X GET "$BASE_URL/api/proprietaire/messages/$CONVERSATION_ID" \
      -H "Authorization: Bearer $TOKEN" | python3 -c "import sys, json; data = json.load(sys.stdin); messages = data.get('data', {}).get('messages', []); unread = [m for m in messages if m.get('destinataireId') == sys.argv[1] and not m.get('estLu')]; print(unread[0]['id'] if unread else '')" "$(curl -s -X GET "$BASE_URL/api/auth/me" -H "Authorization: Bearer $TOKEN" | python3 -c "import sys, json; data = json.load(sys.stdin); print(data.get('data', {}).get('id', ''))" 2>/dev/null)" 2>/dev/null)
